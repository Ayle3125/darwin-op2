...
{
	m_Pan_err_diff = pan - m_PanAngle;
	m_Tilt_err_diff = tilt - m_TiltAngle;
	m_Pan_err = pan ;
	m_Tilt_err = tilt;
	m_PanAngle = pan;
	m_TiltAngle = tilt;
...
void Head::MoveBackTracking()
{
	double pOffset, dOffset;
	m_Pan_err_diff = -m_Pan_err_diff ;
	m_Pan_err +=m_Pan_err_diff ;

	m_Tilt_err_diff = -m_Tilt_err_diff ;
	m_Tilt_err += m_Tilt_err_diff ;
			

	pOffset = -1.0 * m_Pan_err * m_Pan_p_gain;
	pOffset *= pOffset;
	if(m_Pan_err < 0)
		pOffset = -pOffset;
	dOffset = -1.0 * m_Pan_err_diff * m_Pan_d_gain;
	dOffset *= dOffset;
	if(m_Pan_err_diff < 0)
		dOffset = -dOffset;
	m_PanAngle += (pOffset + dOffset);

	pOffset = -1.0 * m_Tilt_err * m_Tilt_p_gain;
	pOffset *= pOffset;
	if(m_Tilt_err < 0)
		pOffset = -pOffset;
	dOffset = -1.0 * m_Tilt_err_diff * m_Tilt_d_gain;
	dOffset *= dOffset;
	if(m_Tilt_err_diff < 0)
		dOffset = -dOffset;
	m_TiltAngle += (pOffset + dOffset);
